  ...
  router.get('/', async (req, res) => {
    ...
    try {
      ...
      const [gameInfo] = await pool.query(
        `select pg.date, pg.white_elo, pg.black_elo, pg.termination, pg.result, pg.time_control, pg.white, pg.black,ob.eco,ob.name from player_games pg left join opening_book ob on pg.book_id = ob.id
 where pg.id= ?`,
        [gameId]
      );
